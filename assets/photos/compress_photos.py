...
- Creates optimized full-size versions in 'big/' subfolder
...
def create_folders():
    ...
    Path('big').mkdir(exist_ok=True)
    Path('thumbnails').mkdir(exist_ok=True)
    print("✓ Created output folders: big/ and thumbnails/")
...
def process_images():
    ...
    for idx, img_file in enumerate(image_files, 1):
        ...
        full_output = Path('big') / output_name
        ...
        if compress_image(img_file, full_output, FULL_SIZE_MAX, FULL_QUALITY):
            ...
            print(f"  ✓ Full size: {full_size:.1f} KB → big/{output_name}")
    ...
    print(f"\nOutput folders:")
    print(f"  - big/      (optimized full-size images)")
...
if __name__ == '__main__':
    ...
    print("2. Move compressed photos from 'big/' to assets/photos/")
